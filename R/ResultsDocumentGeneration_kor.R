...
generateResultsDocumentKor<- function(results, outputFolder, docTemplate="EHDEN", authors = "Author Names", databaseDescription, databaseName, databaseId,smallCellCount,silent=FALSE) {
  ...
  if (is.null(databaseDescription)){
    doc<-doc %>%
      officer::body_add_par(value = paste0("데이터베이스에 대한 간단한 개요를 제공합니다."), style="Highlight")
  } else {
    doc<-doc %>%
  ...
  doc<-doc %>%
    ...
    officer::body_add_par("ETL 문서의 품질에 대하여 데이터 도메인 별로 완전성 및 세부 수준을 검토하고 승인하십시오. 이상적으로는 Rabbit-in-a-Hat 매핑 정의 문서를 기반으로 합니다. 다른 중간 테이블을 활용하는 경우 이에 대하여 자세히 설명하여야 합니다.", style="Highlight") %>%
    ...
    officer::body_add_par("버전 관리 메커니즘이 별도로 존재합니까?", style="Highlight")
  ...
  if (!is.null(results$dataTablesResults)) {
    ...
    doc<-doc %>%
      officer::body_add_gg(plot, height=4) %>%
      officer::body_add_par("그림 2. 시간 경과에 따른 데이터 도메인 당 개인당 레코드 수")

    colnames(results$dataTablesResults$conceptsPerPerson$result) <- c("도메인", "최소", "10%", "25%", "중위 수", "75%", "90%", "최대")
    doc<-doc %>% officer::body_add_break() %>%
      officer::body_add_par(value = "환자당 고유 개념 수", style = "heading 2") %>%
      officer::body_add_par("표 2. 모든 데이터 도메인에 대한 개인당 고유 개념의 수") %>%
  ...
  doc<-doc %>%
    ...
    officer::body_add_par(value = "무작위 검사를 수행하기 위하여, 임의 매핑의 경우 Excel 파일 또는 source_to_concept_map 파일을 보고서와 함께 공유하여야 합니다. 이상적으로 이러한 목록은 소스코드 빈도에 따라 내림차순으로 정렬되어야 합니다.", style = "Highlight")

  vocabResults <-results$vocabularyResults
  if (!is.null(vocabResults)) {
    #vocabularies table

    colnames(vocabResults$conceptCounts$result) <- c("ID", "용어체계 이름", "버전", "S", "C", "-")
    ...
    colnames(vocabResults$vocabularyCounts$result) <- c("테이블명", "개수")

    doc<-doc %>%
      officer::body_add_par(value = "테이블 레코드 수", style = "heading 2") %>%
      officer::body_add_par("표 4. 모든 용어 테이블의 레코드 수") %>%
    ...
    colnames(vocabResults$drugMapping$result) <- c("계열", "레코드 수", "환자 수", "원본 코드 수")
    doc<-doc %>%
      officer::body_add_par(value = "약물 매핑", style = "heading 2") %>%
      officer::body_add_par("표 6. 약물 매핑 수준") %>%
    ...
    doc<-doc %>%
      officer::body_add_par(value = "미 매핑 코드", style = "heading 2")
    ...
    my_mapped_section_kor(doc, vocabResults$mappedDrugs, 13, "약물", smallCellCount)
    my_mapped_section_kor(doc, vocabResults$mappedConditions, 14, "진단", smallCellCount)
    my_mapped_section_kor(doc, vocabResults$mappedMeasurements, 15, "검사", smallCellCount)
    my_mapped_section_kor(doc, vocabResults$mappedObservations, 16, "관찰", smallCellCount)
    my_mapped_section_kor(doc, vocabResults$mappedProcedures, 17, "수술 및 처치", smallCellCount)
    my_mapped_section_kor(doc, vocabResults$mappedDevices, 18, "재료 및 기기", smallCellCount)
  ...
  if (!is.null(results$dataTablesResults)) {
    ...
    doc<-doc %>%
      ...
      officer::body_add_par("표 20. CDM 원본 테이블 내용") %>%
  ...
  if (!is.null(results$performanceResults)) {
    ...
    doc<-doc %>%
      officer::body_add_par(value = "HADES 패키지", style = "heading 2") %>%
      officer::body_add_par("표 21. 설치된 모든 HADES R 패키지의 버전") %>%
  ...
  doc <-  doc %>%
    body_add_par("이 검사 보고서 외에 아래 언급된 모든 항목이 FEEDER-NET과 공유되는지 확인하여 주십시오. 항목을 공유할 수 없는 경우 비고 란에 설명을 기입하여 주십시오.", style = "Normal") %>%
  ...
  writeLines(paste0("해당 문서를 다음 위치에 저장합니다. ",outputFolder,"/",results$databaseId,"-results.docx"))
